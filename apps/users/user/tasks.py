...
# @shared_task(name='Welcome user email')
...
def send_welcome_email(name, email, password, is_notification=False, context = {} ,*args,**kwargs):
    try:
        ...
        configuration_email=json.loads(NotificationType.objects.filter(id=config_id).first().config)
        ...
        html_message = render_to_string(configuration_email['template'],context)
